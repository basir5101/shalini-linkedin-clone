...
const SalesOverview = () => {
  ...
  const optionssalesoverview = {
    ...
    xaxis: {
      type: 'category',
      categories: userInfo?.skill?.length > 0 ? userInfo?.skill?.map(data => data.title) : ['a', 'a', 'a', 'a', 'a', 'a',],
      labels: {
        style: {
  ...
  const seriessalesoverview = [
    ...
    {
      name: 'skills',
      data: userInfo?.skill?.length > 0 ? userInfo?.skill?.map(data => Number(data.value)) : [80, 70, 75, 85, 90, 65],
    },
  ];
  return (
    <DashboardCard
